...
class Data:
    ...
    def __init__(self, data_tag, X_indices, **kwargs):
        """
        Basic parameters
        ----------------
        ...
        self.params = dict((k, v) for k, v in kwargs.iteritems())  # put parameters in a dictionary
        ...
        if "generate_urls" in self.params and self.params["generate_urls"]:
            if len(data_tag) == 2:
                ...
                raise Exception(
                    "Data tag should have first and last url defined if generate_urls parameter is set to true.")
        ...
        if not any(isinstance(self.params["X_indices"], t) for t in [list, xrange]) or not all(
                isinstance(item, int) for item in self.params["X_indices"]) or len(self.params["X_indices"]) == 0:
        ...
        if "y_index" not in self.params or self.params["y_index"] == "":
            self.params[
                "y_index"] = -1  # last feature is used as target label in fitting phase. In prediction phase target label is not outputted if it is set to -1.
        # if value is string and if it it integer
        ...
        # missing_vals param is passed as string
            ...
            self.params["missing_vals"] = set(
                self.params["missing_vals"].split(","))  # missing values symbols are separated with comma.
        ...
        # y_map param is passed as string
            ...
            self.params["y_map"] = self.params["y_map"].replace(" ", "").split(
                ",")  # transformation values are separated with comma.
