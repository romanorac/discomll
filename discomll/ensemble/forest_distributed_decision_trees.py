...
Fit phase
Decision trees algorithm builds one decision tree on a subset of data and it estimates all attributes in every
tree node.
...
Reference
Similar algorithm is proposed in Gongqing Wu, Haiguang Li, Xuegang Hu, Yuanjun Bi, Jing Zhang, and Xindong Wu. MRec4.5:
C4. 5 ensemble classification with mapreduce.
"""
...
def predict(dataset, fitmodel_url, voting=False, save_results=True, show=False):
    ...
    job.run(name="forest_distributed_decision_trees_predict", input=dataset.params["data_tag"],
